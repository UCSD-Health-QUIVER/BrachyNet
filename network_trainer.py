...
class NetworkTrainer:
    ...
    def backward(self, output, target):
        ...
        loss.backward()
        ## update weights every N iterations, if we want to use gradient accumulation
        # if self.log.iter % 15 == 0:
    ...
    def prepare_batch(self,batch):
        if len(batch['hrctv'][tio.DATA].shape) == 4:
            batch['hrctv'][tio.DATA] = batch['hrctv'][tio.DATA].unsqueeze(0)
            batch['dwell_positions'][tio.DATA] = batch['dwell_positions'][tio.DATA].unsqueeze(0)
            batch['bladder'][tio.DATA] = batch['bladder'][tio.DATA].unsqueeze(0)
            batch['rectum'][tio.DATA] =  batch['rectum'][tio.DATA].unsqueeze(0)
            batch['sigmoid'][tio.DATA] = batch['sigmoid'][tio.DATA].unsqueeze(0)
            batch['tandem'][tio.DATA] = batch['tandem'][tio.DATA].unsqueeze(0)
            batch['ovoid'][tio.DATA] = batch['ovoid'][tio.DATA].unsqueeze(0)
            batch['ring_updated'][tio.DATA] = batch['ring_updated'][tio.DATA].unsqueeze(0)
            batch['needle'][tio.DATA] = batch['needle'][tio.DATA].unsqueeze(0)
            batch['true_dose_upsample'][tio.DATA] = batch['true_dose_upsample'][tio.DATA].unsqueeze(0)

        inputs = torch.cat([

        batch['hrctv'][tio.DATA],
        batch['dwell_positions'][tio.DATA],
        batch['bladder'][tio.DATA],
        batch['rectum'][tio.DATA],
        batch['sigmoid'][tio.DATA],
        batch['tandem'][tio.DATA],
        batch['ovoid'][tio.DATA],
        batch['ring_updated'][tio.DATA],
        batch['needle'][tio.DATA]
        ],dim=1).float() 
        targets = batch['true_dose_upsample'][tio.DATA].float()
